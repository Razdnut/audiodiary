...
export default function Stepper({
  ...
  return (
    ...
    >
      <div
        className={cn(
          'mx-auto w-full max-w-md rounded-3xl border border-border/40 bg-background text-foreground shadow-xl',
          stepCircleContainerClassName,
        )}
      >
        <div className={cn('flex w-full items-center gap-3 px-8 pt-8', stepContainerClassName)}>
          ...
          <div className={cn('px-8 pb-8', footerClassName)}>
            <div className={cn('mt-10 flex', currentStep !== 1 ? 'justify-between' : 'justify-end')}>
              ...
              <button
                ...
                className={cn(
                  'flex items-center justify-center rounded-full bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground transition hover:bg-primary/90 active:bg-primary',
                  'disabled:pointer-events-none disabled:opacity-50',
                )}
                {...nextButtonProps}
              >
...
export function Step({ children }: StepProps) {
  return <div className="px-1 pb-8 pt-6 sm:px-4">{children}</div>;
}
...
function StepConnector({ isComplete }: StepConnectorProps) {
  ...
  return (
    <div className="relative mx-2 hidden h-0.5 flex-1 overflow-hidden rounded bg-muted sm:block">
      <motion.div
